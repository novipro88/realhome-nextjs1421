...
const UserProfileDetails = ({ params }) => {
  ...
  const isDisplayAddFriend =
    user?.friends?.includes(data?._id) || user?._id === data?._id;
  return (
      ...
      <div className="mt-5 grid gap-10 lg:grid-cols-[2fr,1fr] items-start">
        ...
        <div className="">
          ...
          <div className="bg-white p-4">
            ...
            <p className="font-bold text-lg lg:text-2xl capitalize">
              {data?.firstName} {data?.lastName}
            </p>
            <p className="text-base lg:text-lg">{data?.occupation}</p>

            {!isDisplayAddFriend && (
              <button className="bg-blue-800 hover:bg-blue-700 text-white font-semibold px-5 rounded-full my-3 py-2">
                Add Friend
              </button>
            )}
          </div>
              ...
              return (
                <div key={friend?._id}>
                  <div className="flex items-center gap-3 justify-between">
                    ...
                    <button
                      className="border-blue-800 border text-blue-800 rounded-full text-xs px-1 hover:bg-blue-800 hover:text-white py-1"
